...
void JointNode::set_joint_y(double min, double init, double max)
{
  ...
  m_joint_y.init = init;
  m_joint_y.max = max;
  m_joint_y.current = 0.0;
  
  Matrix4x4 r = rotationMatrix('z', init);
  m_trans = m_trans*r;
  m_invtrans = r.invert()*m_invtrans;
}
...
void JointNode::rotate(char axis,double angle){
    if(axis == 'z' || axis == 'y'){
        double tmp = m_joint_y.current + angle;
        if( tmp < m_joint_y.min || tmp > m_joint_y.max )
            return;
        else
            m_joint_y.current = tmp;
    }
    else if(axis == 'x'){
        double tmp = m_joint_x.current + angle;
        if( tmp < m_joint_x.min || tmp > m_joint_x.max )
            return;
        else
            m_joint_x.current = tmp;
    }
