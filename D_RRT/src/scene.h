...
class SceneNode 
{
	public:
        ...
        void set_modelstate(std::vector<double> state);
        void set_model_finalstate(Vector3D translation,Vector3D rotation);
        bool deform_model(int id_deformer, const Matrix4x4& ray_transf, const Matrix4x4& inv_deformertransf);
        void reset_deformation();

        //return swift scene from scene_node
        virtual bool fill_swiftscene(SWIFT_Scene *swift_scene);

        static model *m_model;
        static GeometryNode * m_geomodel;
        static std::map <int,GeometryNode*> fixed_objs;
        static bool show_modelcp;
